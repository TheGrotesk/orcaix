...
import type { ShellStage } from '../../src/types.js';

function stage(overrides: Partial<ShellStage>): ShellStage {
  return {
    id: 'test-shell',
    type: 'shell',
    command: 'echo hello',
...
describe('executeShellStage — interactive: true', () => {
  it('runs a quick interactive process and resolves with "interactive session completed"', async () => {
    ...
    const result = await executeShellStage(
      stage({ command: 'true', interactive: true }),
      ctx,
      'unused',
    );
  ...
  it('still resolves (does not reject) when the interactive process exits non-zero', async () => {
    ...
    await expect(
      executeShellStage(stage({ command: 'false', interactive: true }), ctx, 'unused'),
    ).resolves.toBe('interactive session completed');
  });
});
